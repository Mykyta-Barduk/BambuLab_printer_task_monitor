...
import { CheckCircle2, XCircle, RefreshCw, Cpu, Layers, User, Plus, Sliders, MessageSquare, X, Download, Clock } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setAllPrinters(Array.isArray(printersRes.data) ? printersRes.data : []);

      // Оновлюємо внутрішні дані модалки, використовуючи функціональний set, щоб уникнути гонки станів
  ...
  // 🔥 ОНОВЛЕНИЙ ЕФЕКТ: Масив залежностей порожній, таймер живе стабільно і не перезапускає модалку
  useEffect(() => {
    fetchData();
  ...
  return (
    <div className="space-y-6 relative">
      ...
      {activeTab === 'tasks' && (
        <div className="space-y-8">
          <div>
            ...
            {activeJobs.length === 0 ? (
              <div className="p-4 text-xs text-center text-slate-500 bg-slate-900 border border-slate-800 rounded-xl">Наразі активних запусків не виявлено. Поставте gcode на друк.</div>
            ) : (
                  ...
                  <div key={job.id} className={`p-5 bg-slate-900 border rounded-xl space-y-3 ${job.status === 'waiting_confirmation' ? 'border-yellow-500/40 bg-yellow-500/5' : 'border-slate-800'}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {job.remainingMins !== undefined && <span className="text-xs text-slate-400 font-mono flex items-center gap-1"><Clock className="h-3 w-3" /> {job.remainingMins} хв</span>}
                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wide ${job.status === 'waiting_confirmation' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-blue-500/20 text-blue-400 animate-pulse'}`}>
                    ...
                    {job.progress !== undefined && (
                      <div className="w-full bg-slate-950 rounded-full h-1.5 border border-slate-800/40 overflow-hidden">
                        <div className="bg-gradient-to-r from-blue-500 to-purple-500 h-1.5 transition-all duration-500" style={{ width: `${job.progress}%` }} />
                      </div>
                    )}
                    ...
                    {job.status === 'waiting_confirmation' && (
                      <div className="pt-2 grid grid-cols-2 gap-2">
                        <button onClick={() => handleConfirmJob(job.id, true)} className="flex items-center justify-center gap-1.5 bg-emerald-600 text-white font-medium py-2 px-3 rounded-lg text-xs transition-all"><CheckCircle2 className="h-3.5 w-3.5" /> <span>Успішно</span></button>
                        <button onClick={() => handleConfirmJob(job.id, false)} className="flex items-center justify-center gap-1.5 bg-red-600 text-white font-medium py-2 px-3 rounded-lg text-xs transition-all"><XCircle className="h-3.5 w-3.5" /> <span>Брак</span></button>
                      </div>
                    )}
                  </div>
                ))}
          ...
          <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {allPrinters.map((p) => {
                ...
                const isRealPrinting = currentJob && (currentJob.status === 'printing' || currentJob.status === 'waiting_confirmation');
                const showProgress = isRealPrinting && currentJob?.progress !== undefined;
                
                return (
                  <button key={p.id} onClick={() => openPrinterDetails(p, currentJob)} className="p-4 bg-slate-950 hover:bg-slate-900 border border-slate-800 hover:border-purple-500/50 rounded-lg flex flex-col justify-between text-left transition-all">
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-semibold text-sm text-slate-200">{p.name}</span>
                        <span className={`h-2 w-2 rounded-full ${currentJob?.status === 'waiting_confirmation' ? 'bg-amber-500 animate-pulse' : isRealPrinting ? 'bg-blue-500 animate-ping' : 'bg-emerald-500'}`} />
                      </div>
                    ...
                    <div className="w-full mt-3 text-xs flex justify-between text-slate-400 border-t border-slate-900/60 pt-2">
                      ...
                      <span className={`font-mono text-[11px] uppercase font-bold ${currentJob?.status === 'waiting_confirmation' ? 'text-amber-400' : isRealPrinting ? 'text-blue-400' : 'text-emerald-400'}`}>
                        {currentJob?.status === 'waiting_confirmation' ? 'контроль' : isRealPrinting ? `друк ${showProgress ? `(${currentJob.progress}%)` : ''}` : 'вільний'}
                      </span>
                    </div>
                  </button>
                );
      ...
      {/* ==================== 🖨 ДИНАМІЧНИЙ МОНІТОР ПРИНТЕРА (ТЕЛЕМЕТРІЯ З БАЗЫ) ==================== */}
      {selectedPrinterJob && (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-xl shadow-2xl p-6 relative space-y-5">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <span className="text-[11px] uppercase tracking-wider text-slate-500 block font-semibold">Поточний файл друку</span>
              ...
              {/* 🔥 ДИНАМІЧНИЙ ВИВІД: Порожня чи віртуальна джоба з бази теж відмалює прогрес-бар */}
                ...
                <div className="bg-slate-950 border border-slate-850 p-4 rounded-xl grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5 sm:col-span-2">
                    <div className="flex justify-between text-xs font-mono">
                      <span className="text-slate-400 font-bold uppercase">Прогрес завантаження:</span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-sm font-bold text-slate-200 font-mono">{selectedPrinterJob.remainingMins !== undefined ? `${selectedPrinterJob.remainingMins} хв` : 'рахується...'}</span>
                    </div>
                  </div>
